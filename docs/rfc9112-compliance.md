...
| 2.2 | Message parsing robustness | partial | `_connection::tests::test_errors`, `_connection::tests::test_early_detection_of_invalid_request`, `_connection::tests::test_early_detection_of_invalid_response` | Early rejection exists, but broader malformed corpus and fuzz regression coverage should expand. |
...
| 5 | Field syntax and parsing | covered | `_headers::tests::test_normalize_and_validate`, `_headers::tests::test_headers_new_accepts_borrowed_inputs` | Field names/values are validated, normalized for lookup, and raw names are preserved for writing. |
| 5.2 | Obsolete line folding | covered | `_readers::tests::test_obsolete_line_fold_bytes` | Received obs-fold lines are unfolded before header decoding. |
...
| 6.1 | Transfer-Encoding | partial | `_headers::tests::test_normalize_and_validate`, `_connection::tests::test_chunked` | Only `chunked` is supported; unsupported transfer codings are rejected with 501. |
| 6.2 | Content-Length | covered | `_headers::tests::test_normalize_and_validate`, `_connection::tests::test_connection_basics_and_content_length` | Rejects invalid values and conflicting duplicates. |
